...
export default function EditQuotationPage() {
    ...
    const fetchData = useCallback(async () => {
        const [prods, custs, qt] = await Promise.all([
        ...
        setCustomerId(qt.customer?.id || qt.customerId || '');
        setCustomerName(qt.customerName || '');
        setValidUntil(qt.validUntil ? qt.validUntil.slice(0, 10) : '');
        setNotes(qt.notes || '');
        setDiscount(Number(qt.discount || 0));
        setItems(qt.items.map((i: any) => ({
            productId: i.product?.id || i.productId,
            quantity: i.quantity,
            unitPrice: Number(i.unitPrice),
            unitName: i.unitName || i.product?.unit || '',
        })));
        setLoading(false);
    }, [id]);
    ...
    return (
        <div className="max-w-3xl mx-auto animate-fade-in">
            <button onClick={() => router.push(`/quotations/${id}`)} className="text-sm text-gray-500 hover:text-gray-700 mb-3 flex items-center gap-1">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                กลับ
            </button>
            <PageHeader title={`✏️ แก้ไข ${quotationNumber}`} />
