...
export async function getPnLReport(dateFrom?: string, dateTo?: string) {
    ...
    const [revenue, expenses, factoryReturns, withdrawals] = await Promise.all([
        ...
        prisma.stockWithdrawal.aggregate({ where: withdrawalWhere, _sum: { totalAmount: true } }),
    ]);
    ...
    const withdrawalAmount = Number(withdrawals._sum.totalAmount || 0);
    const grossProfit = revenueAmount - cogsAmount;
    const netProfit = grossProfit - expenseAmount - withdrawalAmount;
    ...
    return {
        ...
        withdrawalCost: withdrawalAmount,
        grossProfit,
