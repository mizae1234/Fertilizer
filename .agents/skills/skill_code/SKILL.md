...
- **P&L vs. Profit per Bill (`financial-reports.ts`)**: "Profit per Bill" (กำไรรายบิล) calculates Gross Profit (revenue - COGS) of the selected sales. The "P&L Summary" (สรุป P&L) represents Net Profit, which subtracts general shop expenses (`expenses`) and stock withdrawal costs (`withdrawalCost`) from the Gross Profit. To prevent duplicate return aggregation in `getPnLReport`, we track processed `sale.id`s using a Set, ensuring return quantities are only subtracted once per sale.
